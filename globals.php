...
function getDBConnection() {
	...
	GLOBAL $pdo;
	GLOBAL $debugMode;
	if ($debugMode && $glog) fwrite($glog, "globals.getDBConnection()\r\n");
    ...
    try {
        ...
        if ($debugMode && $glog) fwrite($glog, "PDO Connection Error: " . $e->getMessage() . "\r\n");
        // Handle connection error gracefully - don't expose details to the user in production
        // For development, you might echo or die, but logging is better for production.
...
$db_name = getenv('CHATBOT_DB_NAME') ?: 'chatbot';
$db_user = "";
$db_pwd = "";
	
$projectId 		= getenv('PROJECTID');
if($debugMode && $glog)fwrite($glog, __FILE__." line ".__LINE__.", \$hostmachine=$hostmachine, \$db_host=$db_host, \$db_name=$db_name, \r\n");
...
// This is the same across all applications and databases
$secretId 		= getenv('CHATBOT_DB_USER');
$db_user 		= getSecretValue($projectId, $secretId);
$secretId 		= getenv('CHATBOT_DB_PWD');
$db_pwd 		= getSecretValue($projectId, $secretId);
// if($debugMode && $glog)fwrite($glog, __FILE__." line ".__LINE__.", \$db_user=$db_user, \$db_pwd=$db_pwd, \$projectId=$projectId, \$secretId=$secretId,\r\n");
